...
test('persistent palette restyles the selected node without a right toolbar', async ({ page }) => {
  await boot(page);
...
test('item inspector exposes visual property choices instead of select menus', async ({ page }) => {
  ...
  await expect(page.locator('.property-choice-placement button')).toHaveCount(4);
  const content = page.locator('.property-advanced-group').filter({ hasText: 'Content' });
  await expect(content.locator('.property-advanced-toggle')).toHaveAttribute('aria-expanded', 'false');
  ...
  await content.locator('.property-advanced-toggle').click();
  await page.locator('[data-field="descPlacement"][data-value="below"]').click();
