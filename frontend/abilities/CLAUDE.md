...
- `draggable.ts` — pointer drag → `item.update` Position patches. Drag directly from the renderer's `[data-drag-surface]` (whole card for a node, title bar for a container); no redundant toolbar grip.
