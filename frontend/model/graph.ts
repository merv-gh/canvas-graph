...
export class GraphNode implements NodeEntity {
  ...
  constructor(readonly graph: Graph, readonly id: Id, draft: NodeDraft = {}) {
    ...
    this.Fill = draft.Fill;
    this.BorderColor = draft.BorderColor;
    this.ToggleState = draft.ToggleState;
    this.Description = draft.Description ?? '';
    this.DescriptionPlacement = draft.DescriptionPlacement;
...
export class Graph {
  ...
  /** Resolve an edge endpoint without hard-coding entity kinds. Registered
   *  item stores make containers and future connectable entities addressable
   *  while edges remain compact id-to-id records. */
  refById(id: Id): ItemRef | undefined {
    for (const kind of this.itemStores.keys()) {
      if (kind === 'edge') continue;
      const ref = { kind: kind as ItemRef['kind'], id };
  ...
  private withDefaults(draft: NodeDraft, options: NodeCreateOptions & { nearPosition?: Position }): NodeDraft {
    ...
    if (!hasAnchor && index === 0) {
      return { ...draft, Position: draft.Position ?? { ...anchor } };
    }
    if (hasAnchor) {
      const row = index % 3;
      return {
  ...
  snapshot(): GraphSnapshot {
    ...
    return {
      schemaVersion: 1,
      name: this.name,
      nodes: this.nodes().map(({ id, Label, Position, Size, NodeType, Color, Fill, BorderColor, ToggleState, Description, DescriptionPlacement, ComputeMs, ExpectedRps, LatencyMs, Purpose, Assumptions, Limits, WhatThen, Observability, FailureMode, DataScale, FreshnessMs }) => ({
        id, Label, Position, Size, NodeType, Color, Fill, BorderColor, ToggleState, Description, DescriptionPlacement, ComputeMs, ExpectedRps, LatencyMs, Purpose, Assumptions, Limits, WhatThen, Observability, FailureMode, DataScale, FreshnessMs,
      })),
      edges: this.edges().map(({ id, From, To, Label, EdgeKind, Color, LatencyMs, ThroughputRps, PayloadKb, Purpose, Assumptions, Limits, WhatThen, Observability, FailureMode, DataScale, FreshnessMs }) => ({
        id, From, To, Label, EdgeKind, Color, LatencyMs, ThroughputRps, PayloadKb, Purpose, Assumptions, Limits, WhatThen, Observability, FailureMode, DataScale, FreshnessMs,
      })),
  ...
  replace(snapshot: GraphSnapshot) {
    ...
    snapshot.edges.forEach(draft => {
      // Extension-owned endpoints may restore after the core graph. Keep the
      // relation; renderers safely skip it until both registered items exist.
