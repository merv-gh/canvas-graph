...
import { renderCommandHelp } from './command-help-view';
...
export function registerCommandModal(system: Registry) {
  system('commandModal', (ctx) => {
    ...
    on('help.open', () => emit('modal.open', { title: compactViewport() ? 'Touch guide' : 'Commands and shortcuts', visual: 'command', body: () => renderCommandHelp({ compactViewport, contexts, isInteractionPhase }) }));
