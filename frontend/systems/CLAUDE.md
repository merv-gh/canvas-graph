...
- `telemetry.ts` — OpenTelemetry over the bus. Capture is allocation-free columns (interned name id + typed arrays); batches flush on a microtask into `telemetry.worker.ts`, which runs `core/telemetry-core.ts` (grouping, causality, OTLP, persistence JSON, export POST) off-thread — with an in-process macrotask fallback where `Worker` is absent. Newest 100 spans in `IoApi` as a compact column blob. Export is **off by default** (`telemetry.export.toggle`); no `@opentelemetry/*` dependency.
- `telemetry-portal.ts` — movable/resizable "Event flow" panel drawing those spans live: columns = dispatch depth, links = observed parent→child pairs, nodes light on arrival and fade. Two update paths: structural rebuilds are rate-limited and only fire when the node set changes; heat is an attribute write on the live SVG that emits nothing, so the observer can't observe itself into a loop.
