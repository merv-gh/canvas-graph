...
type CompactShareView = {
  v: [number, number, number];
  s?: [ItemRef['kind'], string, string[]?];
};
type CompactShareViewV2 = [number, number, number, ItemRef['kind']?, string?, string[]?];
const sharePrecision = (value: number) => Math.round(value * 1000) / 1000;
/** Optional presentation state kept separate from the portable graph payload,
 * so old links stay valid and snapshot identity remains view-independent. */
export const encodeShareView = (view: ViewState, selected: ItemRef | null) => bytesToBase64Url(
  new TextEncoder().encode(JSON.stringify([
    sharePrecision(view.x),
    sharePrecision(view.y),
    sharePrecision(view.scale),
    ...(selected
      ? [selected.kind, selected.id, selected.parent] as [ItemRef['kind'], string, string[]?]
      : [] as []),
  ] satisfies CompactShareViewV2)),
);
export const decodeShareView = (encoded: string | null): { view: ViewState; selected: ItemRef | null } | null => {
  if (!encoded) return null;
  try {
    const raw = JSON.parse(new TextDecoder().decode(base64UrlToBytes(encoded))) as unknown;
    const view = Array.isArray(raw) ? raw.slice(0, 3) : (raw as Partial<CompactShareView>)?.v;
    const selected = Array.isArray(raw)
      ? (raw[3] && raw[4] ? [raw[3], raw[4], raw[5]] : undefined)
      : (raw as Partial<CompactShareView>)?.s;
    if (!Array.isArray(view) || view.length !== 3 || !view.every(Number.isFinite) || typeof view[2] !== 'number' || view[2] <= 0) return null;
    if (selected && (!Array.isArray(selected) || typeof selected[0] !== 'string' || typeof selected[1] !== 'string')) return null;
    return {
      view: { x: view[0] as number, y: view[1] as number, scale: view[2] },
      selected: selected ? {
        kind: selected[0] as ItemRef['kind'],
        id: selected[1],
        ...(Array.isArray(selected[2]) && selected[2].length ? { parent: selected[2] as string[] } : {}),
      } : null,
    };
  } catch {
    return null;
...
export const jsonToSnapshot = (source: string): GraphSnapshot | null => {
  try {
...
export const looksLikeMermaid = (text: string) =>
...
export type ImportFormat = 'JSON' | 'Mermaid' | 'Markdown outline';
...
export const markdownOutlineToSnapshot = (source: string): { snapshot: GraphSnapshot; counts: { nodes: number; containers: number } } => {
...
export const looksLikeMarkdownOutline = (source: string) => {
