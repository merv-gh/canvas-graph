...
nohup bash -c "until mkdir '$LOCK' 2>/dev/null; do sleep 2; done; trap \"rmdir '$LOCK' 2>/dev/null\" EXIT; cd '$WT' && BURN=1 BENCH_MAX=1000 BENCH_OUT='$RUNS/$LABEL.md' BENCH_LABEL='$LABEL @ $SHA' npx vitest run --config vitest.bench.config.ts > '$RUNS/$LABEL.log' 2>&1; git -C '$MAIN' worktree remove --force '$WT' 2>/dev/null; git -C '$MAIN' worktree prune" >/dev/null 2>&1 &
