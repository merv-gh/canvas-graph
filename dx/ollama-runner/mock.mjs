...
// model involvement: `node walker/loop.mjs --task zen-canvas --mock`.
...
describe('zen mode keeps stage visible', () => {
  it('stage is pinned to grid-row 2 so zen cannot collapse it', () => {
    const css = readFileSync(resolve(process.cwd(), 'v2/styles.css'), 'utf8');
...
const SCRIPTS = {
  'zen-canvas': [
    { kind: 'tool', name: 'read', args: { path: 'v2/styles.css', from: 175, lines: 20 } },
    { kind: 'tool', name: 'write', args: { path: 'tests/commands/walker/zen-canvas.test.ts', content: ZEN_TEST } },
    { kind: 'tool', name: 'run_test', args: { path: 'tests/commands/walker/zen-canvas.test.ts' } },
    { kind: 'tool', name: 'done', args: { summary: 'red test fails: stage has no grid-row pin' } },
    // --- harness flips to GREEN here ---
    { kind: 'tool', name: 'edit', args: { path: 'v2/styles.css', old: '  grid-column: 2;\n  --grid-size', new: '  grid-column: 2;\n  grid-row: 2;\n  --grid-size' } },
    { kind: 'tool', name: 'run_test', args: { path: 'tests/commands/walker/zen-canvas.test.ts' } },
