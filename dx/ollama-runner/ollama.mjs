...
export const TOOL_SCHEMAS = [
  ...
  { name: 'add_panel', description: 'GREEN: give a system a stage tool panel in ONE call. Inserts declarePanel({id,anchor,…}), widens the ctx destructure, AND routes buttons (adds panel:\'<id>\' to the listed commands\' contribute calls). Supply {system, id, anchor, buttons}. The floating-panel task family (zoom/debug/log).', parameters: { type: 'object', properties: { system: { type: 'string', description: 'frontend/systems/*.ts that owns the panel' }, id: { type: 'string', description: 'panel id, e.g. zoom' }, anchor: { type: 'string', description: 'top-left|top-right|bottom-left|bottom-right' }, buttons: { type: 'array', items: { type: 'string' }, description: 'existing top command ids to route into this panel, e.g. ["view.zoom.in","view.fit.all"]' }, foldId: { type: 'string' }, movable: { type: 'boolean' }, layout: { type: 'string', description: 'stack|toolbar' }, order: { type: 'number' }, mountWhen: { type: 'string', description: 'optional raw arrow expr, e.g. () => debugOn()' } }, required: ['system', 'id', 'anchor'] } },
