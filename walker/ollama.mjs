...
export const TOOL_SCHEMAS = [
  ...
  { name: 'projection', description: 'Compressed architecture view from the workspace. name=commands|events|command-ui|flows|data; filter narrows to an id/file/origin/entity. flows <origin> traces an event origin→handlers→⟳ render leaf (logic/render bugs); data <node|edge|container|item|graph> shows an entity\'s create/update/delete→fact lifecycle. Use before reading whole systems.', parameters: { type: 'object', properties: { name: { type: 'string', enum: ['commands', 'events', 'flows', 'command-ui', 'data'] }, filter: { type: 'string' } }, required: ['name'] } },
